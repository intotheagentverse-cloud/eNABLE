...
async function seedMagnumLifeSciences() {
    console.log('🌱 Seeding Magnum Life Sciences Data...\n');
    ...
    const email = `magnum.admin.${timestamp}@magnumlifesciences.com`;
    const { data: authData, error: authError } = await supabase.auth.signUp({
        email: email,
        password: 'Magnum@2025',
    });
    ...
    const equipmentToCreate = magnumData.equipment_inventory.equipment.map((eq: any) => ({
        ...
        model: eq.name.split(' ').slice(-1)[0], // Extract model from name
    ...
    const qcTestsToCreate = magnumData.qc_monitoring.qc_records.slice(0, 500).map((qc: any) => {
        const equipmentId = equipmentMap[qc.equipment_id];
        const labId = locationMap[qc.location_id];

        // Determine status based on westgard violations
        let status = 'PASS';
    ...
    // Group reagents by name to create reagent master records
    ...
    for (let i = 0; i < magnumData.lab_network.branches.length; i++) {
        ...
        staffMembers.push({
            ...
            employee_id: `MAG-${branch.location_id}-001`,
        ...
        staffMembers.push({
            lab_id: labsData[i + 1].id,
            employee_id: `MAG-${branch.location_id}-002`,
    ...
    console.log('\n✨ Magnum Life Sciences data seeding completed successfully!');
    console.log('\n📧 Login credentials:');
    console.log(`   Email: ${email}`);
    console.log('   Password: Magnum@2025');
    console.log(`\n🏥 Labs created: ${labsData.length}`);
    console.log(`   - ${magnumData.lab_network.lab_name} (HQ)`);
    magnumData.lab_network.branches.forEach((b: any) => console.log(`   - ${b.name}`));
    console.log(`\n⚙️  Equipment created: ${equipmentData.length}`);
    console.log(`📊 Calibration records: ${calibrationData.length}`);
    console.log(`🧪 QC test records: ${qcData.length}`);
    console.log(`💊 Reagent masters: ${reagentData.length}`);
    console.log(`📦 Reagent lots: ${lotData.length}`);
    console.log(`👥 Staff members: ${staffData.length}`);
    console.log(`\n🔑 PRIMARY LAB ID (HQ): ${labsData[0].id}`);
}

seedMagnumLifeSciences()
    .then(() => {
        console.log('\n✅ Seeding complete!');
        process.exit(0);
    })
